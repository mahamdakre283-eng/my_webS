...
class DatabaseService {
  ...
  initialize() {
    ...
    if (window.firebaseConfig && window.firebaseConfig.apiKey && window.firebaseConfig.apiKey !== "YOUR_API_KEY") {
      try {
        ...
        // Storage might not be configured or fail depending on settings, handle gracefully
        try {
      ...
      console.log("No Firebase config found. Using LocalStorage fallback.");
      this.isFirebase = false;
    }
  }
  ...
  async getCategories() {
    ...
    if (this.isFirebase) {
      try {
        ...
        // If timed out, snapshot will be null
  ...
  async getItems() {
    ...
    if (this.isFirebase) {
      try {
        ...
        if (!snapshot) return localFallback;
        if (snapshot.empty) {
          // Add default items in parallel if empty
    ...
    if (this.isFirebase) {
      try {
        ...
        if (doc.exists) {
          ...
          // Initialize settings in Firebase
  ...
  async updateSettings(updatedSettings) {
    ...
    // Then try to sync to Firestore as well
    if (this.isFirebase) {
      try {
        const result = await this.withTimeout(
  ...
  // Helper utility to compress images client-side to keep base64 storage tiny
  ...
  // Image Upload helper (supports Firebase Storage or falls back to Base64)
  async uploadImage(file, storagePath = "images") {
    if (this.isFirebase && this.storage) {
      try {
        // Upload to Firebase Storage with a 3-second timeout race
    ...
    try {
      ...
      // Raw base64 if compression fails
      return new Promise((resolve, reject) => {
        const reader = new FileReader();
...
// Create and expose database service instance globally
